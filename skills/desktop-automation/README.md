...
| [screen-setup](./screen-setup/) | business | One-time onboarding: deps, permissions, model config, gitignore split, map skeleton |
...
The `screen-*` skills are user-invoked (`disable-model-invocation: true`) and share the
same opening — "run the screen-verify discipline … invoke computer-automation by name
alongside this one" (screen-setup is the one-time exception: it configures, it doesn't
verify) — and depend one-way on the layers below. `screen-verify` is model-invoked by
