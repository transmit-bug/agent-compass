...
browser lives in the CLI (`agent-browser skills get core`); this file owns the **records**,
rules, storage, lifecycle, and the orient doc.
...
2. **Determinism in the derive script, judgment in the agent** — status derivation, orient
   rendering, staleness and retention facts are computed by one script
   (`agent-browser-derive`); the agent decides what the computed facts mean. The agent
   writes the records itself — there is no run-lifecycle ceremony.
3. **CLI is the reference** — never hardcode browser command details in skills; load
...
5. **Records only** — the index stores facts (what was done, results), never stage state or
   verdicts about the future; every view is derived by the script (see *Derived views*).
...
| 4 health | `console`, `errors` | page logs and errors — deterministic, zero LLM (`errors` empty = no blocking errors) |
...
## Records — the agent writes them

`.agent-browser/index.json` holds the suite's facts. **The agent writes it directly**
(Write/Edit) as it works — write-as-you-go, exactly like taking notes. The derive script
only reads it, derives from it, and renders the orient doc. Two lists:

### Runs

A **run** is one traceable verification unit for a flow. Keyed per worktree
(`git rev-parse --show-toplevel`) and per app+flow, so parallel projects never collide.

```jsonc
{
  "id": "<app>-<flow>@<startedAt>",        // unique; e.g. "pacsviewer-checkout@2026-08-16T08:00:00Z"
  "app": "<app>",                          // app name (from the suite's own naming)
  "slug": "<flow>",                        // the flow name
  "skill": "web-smoke",                    // owning skill: web-dev|checker|fixer|smoke|logic|maintain
  "branch": "main", "commit": "abc1234",   // from `git branch --show-current` / `git rev-parse HEAD`
  "startedAt": "…", "finishedAt": null,    // ISO-8601 UTC
  "status": "running",                     // running | complete | failed | aborted
  "checkpoints": [                         // write-as-you-go; one verdict per step
    { "step": "…", "method": "snapshot -i", "expected": "…", "actual": "…",
      "verdict": "pass",                   // pass | fail | unsure — nothing else
      "evidence": "path or excerpt" }
  ],
  "ticket": "#14",                         // optional: the flow's tracker ref
  "blockers": [],                          // optional: open refs the flow waits on
  "provenance": ["src/**", "…"],           // optional: source files the verification rests on
  "stale": null, "supersededBy": null      // computed/marked by the derive script
}
```

Rules that bind every run:

- **Write-as-you-go** — record a checkpoint verdict the moment it is known; an interrupted
  run (`status: aborted`) is a legal state with its checkpoints preserved.
- **Verdicts** — exactly one per checkpoint, with evidence. `pass`: the page shows the
  expectation. `fail`: it does not. `unsure`: the page does not clearly resolve — show the
  evidence and ask, never guess. Overall verdict is derived (any fail → fail; else any
  unsure → unsure; else pass) — you never store it.
- **Supersede is a chain, not an overwrite** — when a new run for a flow starts, mark the
  previous terminal run (any non-`running`) `supersededBy = <new id>`; the old entry stays
  until the tidy.
- **Stale is a verdict about the record** — set by the `stale --mark` scan (see below),
  never deleted automatically.

### Assessments

Holistic review per scope: `{app, scope, assessedAt, completeness, logic, flow, ui, gist}`.
Scope is a feature name, or `app` for app-level concerns. Kinds: `completeness:
complete|partial|missing`, `logic: clear|ambiguous|contradictory`, `flow:
sound|awkward|broken`, `ui: reasonable|ineffective|broken`. `gist` is the one-line summary;
durable detail is linked, not pasted (see *Anti-rot*).

## Derived views — never hand-maintained

Everything else is **computed by `agent-browser-derive render`**, never stored: the status
table, next actions, and the orient doc. Status is derived per app+flow from the latest
record, worst first:

| Status | Derived when (latest run per app+flow) |
|---|---|
| not-started | no run recorded (no record → no row; derive from open tracker tickets) |
| in-progress | latest run `running` / `aborted`, or derived verdict `unsure` |
| blocked | latest run carries `blockers` |
| stale | latest run flagged `stale` by the scan |
| failing | latest run `failed`, or derived verdict `fail` |
| verified | latest run `complete` and derived verdict `pass` |
...
`index.md` is the **orient doc**: derived by `agent-browser-derive render`, never
hand-edited. A fresh session reads it first (web-router's orient step), then acts. Sections:
...
│   ├── index.json             # records — WRITTEN BY THE AGENT (schema above)
│   ├── index.md               # orient doc, derived by agent-browser-derive render
│   ├── scripts/               # agent-browser-derive (bootstrapped by web-setup)
...
`docs/agent-browser/` (git-tracked) while `.agent-browser/` holds state. Each scenario is
one markdown document with frontmatter; the suite never duplicates one meaning across two
scenarios.
...
Defaults apply when a key is missing (they match the derive script's `DEFAULTS`).
`uiSurface` globs, `uiKeywords`, and `uiDeps` feed the staleness scan; `thresholds` feed
both the scan and the retention tidy.

## The derive script

`agent-browser-derive` ships with **web-verify** and is bootstrapped by web-setup into the
app repo's `.agent-browser/scripts/`. It only computes and stores — it never decides. Run
from the app repo root:

| Command | Effect |
|---|---|
| `agent-browser-derive init` | scaffold `.agent-browser/` (empty records + orient doc) and append `.agent-browser/runs/` to `.gitignore` — idempotent |
| `agent-browser-derive render` | derive status + next actions from records; render `index.md`; report schema-drift warnings (e.g. an out-of-enum checkpoint verdict) |
| `agent-browser-derive stale [--since <days>] [--mark]` | deterministic staleness scan; `--mark` records the facts on the records and re-renders |
| `agent-browser-derive tidy [--dry-run]` / `--apply` | selective forgetting per retention policy; `--apply` removes the records and their evidence together |

Staleness signals: **strong** — changed files intersect the run's `provenance`
...
`structure/` in the run's dir. Screenshots under `shots/` are for the visual branch and for
human review. Reports never end silently. Trust: screenshots may carry sensitive content —
auth through the vault (`agent-browser auth login <name>`), never echo secrets; launch
sensitive targets with `--allowed-domains` to contain the browser.
